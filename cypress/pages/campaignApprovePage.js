...
export default class CampaignApprovePage {
  constructor(campaign) {
    ...
    this.advertiser_category = campaign.campaign_approve.advertiser_category
      ...
      : null
    this.budget_pacing = campaign.campaign_approve.budget_pacing
    this.exchange = campaign.campaign_approve.exchange
    this.app_site_category_value_to_exclude = campaign.campaign_approve.app_site_category_value_to_exclude
      ? campaign.campaign_approve.app_site_category_value_to_exclude
      : null
  }
  ...
  approveCampaignAndVerifyCampaignStatus() {
    ...
    this.app_site_category_value_to_exclude ? this.selectAppSiteCategory() : null
    cy.get(CAMPAIGN_APPROVE.BUDGET_PACING).select(CONSTANTS.BUDGET_PACING[this.budget_pacing.toLowerCase()], {
      force: true,
    })
  ...
  selectAppSiteCategory(exclude = true) {
    cy.get(CAMPAIGN_APPROVE.APP_SITE_CATEGORY).select(this.app_site_category_value_to_exclude, { force: true })
    if (exclude) {
      cy.get(CAMPAIGN_APPROVE.APP_SITE_CATEGORY_EXCLUDE_RADIO_INPUT).click()
    } else {
      cy.get(CAMPAIGN_APPROVE.APP_SITE_CATEGORY_INCLUDE_RADIO_INPUT).click()
    }
  }
